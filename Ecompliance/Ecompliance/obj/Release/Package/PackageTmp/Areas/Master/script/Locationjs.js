...
function OnSuccessPage(e) {
    HandleResponse(e, "dvLocation", "btnReset", "LocationID");
    if (e.IsSuccess == true) {
        var grid = $("#dvLocationgrid").data("kendoGrid");
...
function HistoryGridData(LocationID) {
    $.ajax({
        type: "GET",
        url: GetLocationHistoryUrl,
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.IsSuccess) {
                if (kgrid != "") {
                    $('#dvLocationgrid').kendoGrid('destroy').empty();
                }
                var objAddDiv = $("#dvAdd");
                ...
                kgrid = $("#dvLocationgrid").kendoGrid({
                    dataSource: {
                        pageSize: 15,
                    ...
                    columns: [
                               ...
                               { field: "Name", title: "Location", width: 200 },
                ...
                if (currentPage != 0)
                    $('#dvLocationgrid').data().kendoGrid.dataSource.page(currentPage);
            }
            else {
                FailResponse(response);
            }
...
var ViewHandler = function ViewHandler(e) {
    ...
    $.get(GetLocationUrl, { LocationID: LocationID }, function (response) {
        //console.log(response);
        if (response.IsSuccess) {
            var data = JSON.parse(response.Data)[0];
            $("#lblLocation_Name").html(data.Name);
            $("#lblState_Name").html(data.State);
            var IsAct = data.IsAct;
            (IsAct == 1) ? $('#lblIsAct').html('Active') : $('#lblIsAct').html('In Active');
            HistoryGridData(LocationID);
        }
        else {
            FailResponse(response);
        }
