...
$(document).ready(function () {
    ...
    $("#btnSendToAuditor").click(function () {
        ...
        UpdateAll('Submitted to Client / Auditor');
    });
...
function CalculateAvgSentToAuditor(Site, Status) {
    //var Obj = FilterFunction(Site, Role); Resubmitted Document
    var Obj = DataSendToAuditor.filter(function (item) {
...
function CalculateNotSent(Site, Status) {
    //var Obj = FilterFunction(Site, Role);
    var Obj = DataSendToAuditor.filter(function (item) {
        return item.Site == Site && (item.AuditStatus == Status ||item.AuditStatus=='Ready For Resubmission');
    });//     FilterFunction(Site, Role);
    var ObjComp = DataSendToAuditor.filter(function (item) {
        return item.Site == Site;
    });//     FilterFunction(Site, Role);
...
function CalculateAvgSentToAuditorComp(Comp, Status) {
    //var Obj = FilterFunction(Site, Role); Resubmitted Document
    var Obj = DataSendToAuditor.filter(function (item) {
...
function CalculateNotSentComp(Comp, Status) {
    //var Obj = FilterFunction(Site, Role);
    var Obj = DataSendToAuditor.filter(function (item) {
        return item.Company == Comp && (item.AuditStatus == Status || item.AuditStatus == 'Ready For Resubmission');
    });//     FilterFunction(Site, Role);
    var ObjComp = DataSendToAuditor.filter(function (item) {
        return item.Company == Comp;
    });//     FilterFunction(Site, Role);
            ...
            if (data.IsSuccess == true) {
                ...
                $("#grid").kendoGrid({
                    ...
                    columns: [
                        ...
                        { field: "Company", hidden: true, title: "Company", groupHeaderTemplate: "#=value #<br/>  <span style='margin: 8px 8px 8px 21px;display: inline-block;color:mediumpurple;'>Not Submitted to Client / Auditor:(#=CalculateNotSentComp(value,'Not Submitted to Client / Auditor')#%)</span><span style='margin-right: 9px;color:red;'>Rejected by Client / Auditor:(#=CalculateAvgComp(value,'Rejected by Client / Auditor')#%)</span><span style='margin-right: 9px;color:blue;'>Submitted to Client / Auditor:(#=CalculateAvgSentToAuditorComp(value,'Submitted to Client / Auditor')#%)</span><span style='margin-right: 9px;color:green;'>Viewed / Approved by Client / Auditor:(#=CalculateAvgComp(value,'Viewed / Approved by Client / Auditor')#%)</span>" }, 
                        //{ field: "Site", hidden: true, title: "Site", groupHeaderTemplate: "# if( aggregates.Percentage.average == null ) {# #=value #  : (Average : -)#} else {# #=value #  : (Average : #=kendo.format('{0:n}',aggregates.Percentage.average)#%)#} #" },
                        { field: "Site", hidden: true, title: "Site", groupHeaderTemplate: "#=value #<br/>  <span style='margin: 8px 8px 8px 21px;display: inline-block;color:mediumpurple;'>Not Submitted to Client / Auditor:(#=CalculateNotSent(value,'Not Submitted to Client / Auditor')#%)</span><span style='margin-right: 9px;color:red;'>Rejected by Client / Auditor:(#=CalculateAvg(value,'Rejected by Client / Auditor')#%)</span><span style='margin-right: 9px;color:blue;'>Submitted to Client / Auditor:(#=CalculateAvgSentToAuditor(value,'Submitted to Client / Auditor')#%)</span><span style='margin-right: 9px;color:green;'>Viewed / Approved by Client / Auditor:(#=CalculateAvg(value,'Viewed / Approved by Client / Auditor')#%)</span>" },
                    ...
                    dataBound: function (e) {
                        ...
                        for (var i = 0; i < gridData.length; i++) {
                            ...
                            if (gridData[i].AuditStatus == "Viewed / Approved by Client / Auditor" || gridData[i].AuditStatus == "Pending with Client / Auditor" || gridData[i].AuditStatus == "Resubmitted Document" || (gridData[i].AuditStatus == "Rejected by Client / Auditor" && Role == 'Auditor') || (gridData[i].AuditStatus == "Submitted to Client / Auditor" && Role == 'Auditee')) { // || gridData[i].AuditStatus == "Rejected by Auditor"
                    ...
                    //    }
                    //    else {
                    //        alert(e.filter.logic);
                    //        alert(e.filter.filters[0].field);
                    //        alert(e.filter.filters[0].operator);
...
function UpdateAll(type) {
    ...
    if (lstSendToAuditor.length > 0) {
        ...
        $.post("SetAuditee_Auditor", { Model: lstSendToAuditor, Type: type }, function (response) {
            if (response.IsSuccess) {
...
$(document).ready(function () {
    $("#btnUploadSubmit").click(function () {
        var Remarks = $("#txtUploadRemarks").val();
        var Docid = GlobalDOCID;
        $.post("SetUploaderRemarks", { Remarks: Remarks, Docid: Docid }, function (response) {
            if (response.IsSuccess) {
                createSuccessPOP(response.Message);
                BindScoreCardGrid();
                $("#log").dialog("close");
...
function BindAttachGrid(Data1) {
    ...
    var GridColumns = [
        ...
        //{ field: "makerRemark", title: "Maker Remarks" },
    ];
    //if (objAddDiv.length > 0) {
    var cmd = {
        command: [{ name: "Delete", width: "25px", text: "", imageClass: "k-icon k-filedelete", click: Deletefile, title: "Delete" },
            { name: "Download", text: "", imageClass: "k-icon k-filedownload", click: Download, title: "Download" }
        ], title: "Action"
    };
...
function AttachDocPopUp(e) {
    ...
    var Status = dataItem.AuditStatus;
    if (Status == 'Rejected by Client / Auditor')
        $("#dvRemakrs").show();
    else
        $("#dvRemakrs").hide();
    GetTaskFiles(GlobalDOCID);
