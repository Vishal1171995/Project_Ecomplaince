...
function BindScoreCardGrid() {
    ...
    BindContractorGrid(Comp, Site, Month, Year);
    if (EmptyCount >= 2) {
        $("#dvReport").hide();
        $("#dvNoRecord").show();
    }
        ...
        success: function (data) {
            if (data.IsSuccess == true) {
                ...
                , dataBound: function (e) {
                    ...
                    columns: [
                                { field: "Company", title: "Company", groupHeaderTemplate: "# if( aggregates.Percentage.average == null ) {# #=value #  : (Average : -)#} else {# #=value #  : (Average : #=kendo.format('{0:n}',aggregates.Percentage.average)#%)#} #" },
                                { field: "Site", title: "Site", groupHeaderTemplate: "# if( aggregates.Percentage.average == null ) {# #=value #  : (Average : -)#} else {# #=value #  : (Average : #=kendo.format('{0:n}',aggregates.Percentage.average)#%)#} #" },
                                { field: "Act", title: "Act", groupHeaderTemplate: "# if( aggregates.Percentage.average == null ) {# #=value #  : (Average : -)#} else {# #=value #  : (Average : #=kendo.format('{0:n}',aggregates.Percentage.average)#%)#} #" },
...
function BindContractorGrid(Comp, Site, Month, Year) {
    var str = { "CompanyID": Comp, "SiteID": Site, "Month": Month, "Year": Year, "Type": "Contractor" };
    $.ajax({
        type: "GET",
        url: GetScoreCardDataUrl,
        contentType: "application/json; charset=utf-8",
        ...
        success: function (data) {
            if (data.IsSuccess == true) {
                ...
                , columns: [
                            ...
                            { field: "Site", title: "Site", groupHeaderTemplate: "# if( aggregates.Percentage.average == null ) {# #=value #  : (Average : -)#} else {# #=value #  : (Average : #=kendo.format('{0:n}',aggregates.Percentage.average)#%)#} #" },
                            { field: "Act", title: "Act", groupHeaderTemplate: "# if( aggregates.Percentage.average == null ) {# #=value #  : (Average : -)#} else {# #=value #  : (Average : #=kendo.format('{0:n}',aggregates.Percentage.average)#%)#} #" },
                            { field: "Contractor", title: "Contractor", groupHeaderTemplate: "# if( aggregates.Percentage.average == null ) {# #=value #  : (Average : -)#} else {# #=value #  : (Average : #=kendo.format('{0:n}',aggregates.Percentage.average)#%)#} #" },
                            { field: "Activity", title: "Activity" },
                            { field: "PE", title: "PE" },
                            { field: "Percentage", title: "%", aggregates: ["average"], type: "number" },
                            { field: "DueDate", title: "Due Date" },
